...
const generateToken = async (data) => jwt.sign(data, privateKey, {expiresIn: '3h'})

const generateRefreshToken = async (id) => {
    const refreshToken = randToken.uid(256);
    await redisPool.setAsync(refreshToken, id, 'EX', 120 * 60 );
    return refreshToken;
};
...
const extendSession = async (key) => {
    ...
    if (!isEmpty(id)) await redisPool.setAsync(key, id, 'EX', 120 * 60)
}
